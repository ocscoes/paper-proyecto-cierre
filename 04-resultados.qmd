...
datos_fit <- datos_merge %>%
  ...
  select(cohesion_general_ind, cohesion_vertical_ind, cohesion_horizontal_ind,
         ...
         pib_we, gini_we, dem_we, wgi_we, mig_we, edu_we,fra_we,
         pib_be, gini_be, dem_be, wgi_be, mig_be, edu_be,fra_be,
...
names(datos_merge)
datos_merge$fe_etfra <- as.numeric(stringi::stri_replace_all(datos_merge$fe_etfra,regex = ",",replacement = "."))

datos_merge %>% select(cohesion_horizontal_ind,cohesion_vertical_ind,
                       gini_index,pib_per_capita_ppa,
                       indice_v_dem,wgi,
                       percent_pob_migrante,fe_etfra) %>% tab_corr(triangle = "upper")
...
micro <- lmer(cohesion_horizontal_ind ~ 1 + nivel_educ + sexo + edad_c + I(edad_c^2) + pos_politica + ola_s + (1 | country_wave) + (1 | pais), data= datos_fit, weights= wt)
...
micro <- lmer(cohesion_vertical_ind ~ 1 + nivel_educ + sexo + edad_c + I(edad_c^2) + pos_politica + ola_s + (1 | country_wave) + (1 | pais), data= datos_fit, weights= wt)
macro_gini <- update(micro,. ~ . + gini_be + gini_we)
macro_gdp <- update(micro,. ~ . + pib_be + pib_we)
macro_econ <- update(micro,. ~ . + gini_be+gini_we+pib_be+pib_we)
v_econ <- list(macro_gini,macro_gdp,macro_econ)

htmlreg(c(h_econ,v_econ),
...
h_demo <- list(macro_mig,macro_fra,macro_demo)

micro <- lmer(cohesion_vertical_ind ~ 1 + nivel_educ + sexo + edad_c + I(edad_c^2) + pos_politica + ola_s + (1 | country_wave) + (1 | pais), data= datos_fit, weights= wt)
macro_mig <- update(micro,. ~ . + mig_be + mig_we)
macro_fra <- update(micro,. ~ . + fra_be + fra_we)
macro_demo <- update(micro,. ~ . + mig_be+mig_we+fra_be + fra_we)
v_demo <- list(macro_mig,macro_fra,macro_demo)

htmlreg(c(h_demo,v_demo),
        # file = "output/table003.html",
        caption.above = T,
        caption = "Multilevel regression for demographic macro factors and social cohesion", 
          custom.coef.map = 
            list(mig_be ="Migration (BE)",mig_we = "Migration (WE)",
                 fra_be = "Fractionalization (BE)",fra_we = "Fractionalization (wE)"),
          ,custom.header = list("Horizontal"=1:3,"Vertical"=4:6),
          custom.gof.rows = 
            list(Controls=c("Yes","Yes","Yes","Yes","Yes","Yes"))          
          )
