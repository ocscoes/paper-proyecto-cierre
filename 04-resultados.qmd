...
fig_gini_h <- 
  ...
  # geom_smooth(data = df_sum_la, method = lm, se = FALSE, aes(mid_wid_gini_disp, y = mid_cohesion_horizontal), color = "pink") +
...
micro_h <- lmer(cohesion_horizontal_ind ~ 1 + nivel_educ + sexo + edad_c + I(edad_c^2) + pos_politica + ola + I(ola^2)+ (1 | country_wave) + (1 | pais), data= datos_fit, weights= wt)
micro_v <- lmer(cohesion_vertical_ind ~ 1 + nivel_educ + sexo + edad_c + I(edad_c^2) + pos_politica + ola + I(ola^2) + (1 | country_wave) + (1 | pais), data= datos_fit, weights= wt)
...
htmlreg(c(micro_h,micro_v),single.row = F,digits = 3,
       ...
       groups = 
         list("Education (ref.=Primary)"=2:3,
