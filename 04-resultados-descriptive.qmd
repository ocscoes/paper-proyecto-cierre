...
corr_table_hor <- df_sum %>%
  select(iso2c, all_of(y_var), all_of(x_vars)) %>%
  pivot_longer(
    cols = all_of(x_vars),
    names_to = "x_var",
    values_to = "x"
  ) %>%
  filter(!is.na(x), !is.na(.data[[y_var]])) %>%
  group_by(iso2c, x_var) %>%
  summarise(
    n = n(),
    test = list(if (n() >= 3) cor.test(x, .data[[y_var]], method = "pearson") else NULL),
    .groups = "drop"
  ) %>%
  mutate(
    r = map_dbl(test, ~ if (is.null(.x)) NA_real_ else unname(.x$estimate)),
    p = map_dbl(test, ~ if (is.null(.x)) NA_real_ else .x$p.value)
  ) %>%
  select(iso2c, x_var, r, p) %>%
  pivot_wider(
    names_from = x_var,
    values_from = c(r, p),
    names_glue = "{.value}_{x_var}"
  ) %>%
  arrange(iso2c) |>
  select(iso2c,
         ends_with("disp"),
